...
from controller.room_type_image_controller import get_Froom_images
from controller.room_type_controller import get_room_types


froom_Images: list[room_images] = get_Froom_images()
...
for i in froom_Images:
    room_gallery.append(f'{HOST}api/files/obbhx5i2333jgko/{froom_Images[index].id}/{froom_Images[index].room_image}?token=')
    index += 1
for e in types:
    if e.type == 'Family':
        price = e.price
...
family_rooms_list = ft.Row([], wrap=True, alignment=ft.MainAxisAlignment.SPACE_EVENLY)
available_room_label = ft.Text(value='', size=10, color='#525252')

def append_Froom_to_row(q):
    Froom_list = q
    counter.clear()
    room_status.clear()
    family_rooms_list.controls.clear()
    for froom in Froom_list:
        if froom.status == False and froom.room_type == '4yjezbow65i8fy6':
            family_rooms_list.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Row(
                                [
                                    ft.Text(value=f'Room {froom.room_number}', color='black'),
                                    ft.Container(
                                        content=ft.Text(value='F', color='white', font_family='Arial', size=12, weight=ft.FontWeight.BOLD),
                    ...
                    width=250,
                    height=120,
                    border=ft.border.only(left=ft.border.BorderSide(3, color='#1A69E8'),
            ...
            counter.append(froom.room_type)

        elif froom.status == True and froom.room_type == '4yjezbow65i8fy6':
            family_rooms_list.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
...
description_text = """
    The Family Room comprises of 1 Double bed or 2 Twin Beds, 2 Bedside Tables, a Desk & Chair.
    This room is larger than the Standard Room and is therefore spacious enough for an additional
    bed for triple occupancy. (Triple Room). The room is furnished with wall to wall
    carpeting, trendy furnishings and a balcony. Our ultramodern glass bathroom is equipped
    with hairdryer, magnifying shaving and make up mirror as well as all the amenities you
    could possible need during your stay. A Complimentary Bottle of Wine, Fresh Fruit and Mineral
    Water, are provided on arrival. Electric current: 220 Volts. Smoking rooms are also available.
"""
...
class family_room_view(ft.UserControl):
    def build(self):
        return ft.Container(
            content=ft.Column(
                [
                    ft.Container(
                        content=ft.Text(value='Family-Room Gallery', color='black',font_family='Consolas', size=30),
                        width=908,
                        height=70,
                        bgcolor="#F4F4F4",
                        alignment=ft.alignment.center
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ft.Container(
                                    content=ft.Text(value='FAMILY ROOM AMENITIES', color='black',font_family='Consolas', size=20),
                                    alignment=ft.alignment.center,
                                    bgcolor='#F4F4F4'
                                ),
                                ft.Container(
