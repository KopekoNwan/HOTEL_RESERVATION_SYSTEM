...
from controller.room_type_image_controller import get_Sroom_images
from controller.room_type_controller import get_room_types



sroom_Images: list[room_images] = get_Sroom_images()
...
for i in sroom_Images:
    room_gallery.append(f'{HOST}api/files/obbhx5i2333jgko/{sroom_Images[index].id}/{sroom_Images[index].room_image}?token=')
    index += 1
...
standard_rooms_list = ft.Row([], wrap=True, alignment=ft.MainAxisAlignment.SPACE_EVENLY)
available_room_label = ft.Text(value='', size=10, color='#525252')

def append_Sroom_to_row(f):
    Sroom_list = f
    counter.clear()
    room_status.clear()
    standard_rooms_list.controls.clear()
    for sroom in Sroom_list:
        if sroom.status == False and sroom.room_type == 'v54ixd0f7vlu3kq':
            standard_rooms_list.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Row(
                                [
                                    ft.Text(value=f'Room {sroom.room_number}', color='black'),
                                    ft.Container(
                                        content=ft.Text(value='S', color='white', font_family='Arial', size=12, weight=ft.FontWeight.BOLD),
                    ...
                    width=250,
                    height=120,
                    border=ft.border.only(left=ft.border.BorderSide(3, color='#323232'),
            ...
            counter.append(sroom.room_type)

        elif sroom.status == True and sroom.room_type == 'v54ixd0f7vlu3kq':
            standard_rooms_list.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
...
class standard_room_view(ft.UserControl):
    def build(self):
        return ft.Container(
            content=ft.Column(
                [
                    ft.Container(
                        content=ft.Text(value='Standard-Room Gallery', color='black',font_family='Consolas', size=30),
                        width=908,
                        height=70,
                        bgcolor="#F4F4F4",
                        alignment=ft.alignment.center
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ft.Container(
                                    content=ft.Text(value='STANDARD ROOM AMENITIES', color='black',font_family='Consolas', size=20),
                                    alignment=ft.alignment.center,
                                    bgcolor='#F4F4F4'
                                ),
                                ft.Container(
